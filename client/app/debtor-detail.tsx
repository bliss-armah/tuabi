...
export default function DebtorDetail() {
  ...
  const handleAddPayment = async () => {
    ...
    try {
      ...
      await debtorService.addPayment(Number(id), {
        amount_changed: finalAmount,
        note: paymentNote || null
      });
      
      // Refresh debtor details
      loadDebtorDetails(Number(id));
      ...
      Alert.alert(
        'Success', 
        isAddingDebt ? 'Debt added successfully' : 'Payment recorded successfully'
      );
    } catch (err) {
    ...
    Alert.alert(
      ...
      [
        ...
        { 
          ...
          onPress: async () => {
            try {
              await debtorService.deleteDebtor(Number(id));
              Alert.alert(
                'Success',
  ...
  if (isLoading) {
    return (
      <View style={[styles.loadingContainer, { backgroundColor: Colors[theme].background }]}>
        ...
        <Text style={[styles.loadingText, { color: Colors[theme].text }]}>Loading debtor details...</Text>
      </View>
    );
  }
  ...
  if (error) {
    return (
      <View style={[styles.errorContainer, { backgroundColor: Colors[theme].background }]}>
        <Text style={styles.errorText}>{error}</Text>
        <TouchableOpacity 
          style={[styles.retryButton, { backgroundColor: Colors[theme].primary }]}
          onPress={() => loadDebtorDetails(Number(id))}
        >
    ...
    <View style={[styles.container, { backgroundColor: Colors[theme].background }]}>
      <ScrollView>
        <View style={[styles.header, { backgroundColor: Colors[theme].primary }]}>
          ...
          <TouchableOpacity 
            style={[styles.editButton, { backgroundColor: Colors[theme].secondary }]}
        ...
        <View style={[styles.debtorInfoCard, { backgroundColor: Colors[theme].card, borderColor: Colors[theme].border }]}>
          <View style={[styles.amountContainer, { borderBottomColor: Colors[theme].border }]}>
            <Text style={[styles.amountLabel, { color: Colors[theme].text }]}>Total Amount Owed</Text>
            <Text style={[
              styles.amountValue,
              debtor.amount_owed > 0 ? styles.positiveAmount : styles.zeroAmount
            ]}>
          ...
          {debtor.phone_number && (
            <View style={styles.infoRow}>
              ...
              <TouchableOpacity 
                style={[styles.callButton, { backgroundColor: Colors[theme].primary }]}
          ...
          <TouchableOpacity 
            style={[styles.actionButton, { backgroundColor: Colors[theme].secondary }]}
            onPress={() => {
        ...
        <View style={[styles.historyContainer, { backgroundColor: Colors[theme].card }]}>
          <Text style={[styles.historyTitle, { color: Colors[theme].text }]}>Payment History</Text>
            ...
            debtor.history.map((item) => (
              <View key={item.id} style={styles.historyItem}>
                <View style={styles.historyHeader}>
                  <View style={styles.historyLeft}>
                    ...
                    <Text style={styles.historyAction}>
                      {item.action === 'add' ? 'Debt Added' : item.action === 'reduce' ? 'Payment Received' : 'Settled'}
                    </Text>
                  </View>
                ...
                <Text style={[
                  ...
                  item.action === 'add' ? styles.positiveAmount : styles.negativeAmount
                ]}>
                  {item.action === 'add' ? '+' : '-'}${Math.abs(item.amount_changed).toFixed(2)}
                </Text>
